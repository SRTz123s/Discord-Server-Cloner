...
# config loading (optional, GUI manages it)
logs_enabled = True
try:
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        _data = __import__("json").load(f)
        logs_enabled = _data.get("logs", True)
except Exception:
    pass
    ...
    async def channels_create(guild_to: discord.Guild,
        ...
        for channel in channels:
            ...
            try:
                ...
                logs(f"Error While Creating Channel {channel.name}: {error}",
                     'error')
